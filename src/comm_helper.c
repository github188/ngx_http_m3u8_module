...
int h264_nalu_annexb_get(unsigned char *frame, int length, NALU_t *nalu)
{
	...
	info2 = h264_nalu_startbit2 (frame);    //Check whether Buf is 0x000001
	if(info2 != 1) 
	{
		info3 = h264_nalu_startbit3 (frame);//Check whether Buf is 0x00000001
...
//			memcpy (nalu->buf, &frame[nalu->startcodeprefix_len], nalu->len); 
			nalu->buf = (char*)frame + nalu->startcodeprefix_len;  
		...
		pos++;
		info3 = h264_nalu_startbit3(&frame[pos-4]);			 //Check whether Buf is 0x00000001
		if(info3 != 1)
			info2 = h264_nalu_startbit2(&frame[pos-3]);			 //Check whether Buf is 0x000001
