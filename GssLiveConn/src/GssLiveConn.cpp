...
bool GssLiveConn::Start()
{
	...
	if (busc)
	{
		LOG_INFO("Gssliveconn start success, server = %s, port = %d, uid = %s",m_server,m_port,m_uid);
	}
	else
	{
		LOG_ERROR("Gssliveconn start failed, server = %s, port = %d, uid = %s",m_server,m_port,m_uid);
	}
...
bool GssLiveConn::GetVideoFrame( unsigned char** pData, int &datalen, unsigned int &timestamp)
{
	bool bSuc = false;
//	printf("********%p GetVideoFrame count = %d\n",this,m_countsVideo);
	if (m_countsVideo > 0 && pData && *pData == NULL)
	{
		*pData = m_bufferVideo[m_curVideoIndex]->m_buffer;
		...
		timestamp = m_bufferVideo[m_curVideoIndex]->m_frameHeader.nTimestamp;
		bSuc = true;
	}
	return bSuc;
}
...
bool GssLiveConn::GetAudioFrame( unsigned char** pData, int &datalen, unsigned int &timestamp)
{
	bool bSuc = false;
...
void GssLiveConn::OnDisconnect( void *transport, void* user_data, int status )
{
	...
	if(pConn)
	{
		...
		pConn->Stop();
		LOG_INFO("GssLiveConn::OnDisconnect transport = %p, user_data = %p, status = %d,uid = %s",transport,user_data,status,pConn->m_uid);
	}
	else
	{
		LOG_ERROR("GssLiveConn::OnDisconnect transport = %p, user_data = %p, status = %d",transport,user_data,status);
	}
}
...
void GssLiveConn::OnConnectResult( void *transport, void* user_data, int status )
{
	...
	if(pConn)
	{
		...
		LOG_INFO("GssLiveConn::OnConnectResult ,status = %d, conn = %p,uid = %s",status,user_data,pConn->m_uid);
	}
	else
	{
