...
public:
	...
	bool GetVideoFrame(unsigned char** pData, int &datalen, unsigned int &timestamp); //成功后，当使用完pData之后必须调用FreeVideoFrame
	void FreeVideoFrame();
	bool GetAudioFrame(unsigned char** pData, int &datalen, unsigned int &timestamp);
	void FreeAudioFrame();
